...
public class OwnerService implements OwnerExternalAPI, OwnerPublicAPI {
    ...
    public void savePet(Pet pet) {
        ...
        if (pet.getId() != null) {
            Optional<OwnerPet> existingPetOpt = ownerPetRepository.findById(pet.getId());
            if (existingPetOpt.isPresent()) {
                existingPet = existingPetOpt.get();
                // Garantir que o owner_id não seja alterado durante a atualização
                if (!existingPet.getOwner_id().equals(pet.getOwner_id())) {
                    System.out.println("AVISO: Tentativa de alterar owner_id do pet de " + 
                                      existingPet.getOwner_id() + " para " + pet.getOwner_id() + 
                                      ". Mantendo o owner_id original.");
                    pet.setOwner_id(existingPet.getOwner_id());
                }
            }
        }
        ...
        // Atualizar o owner para manter a associação com o pet
        Owner owner = ownerRepository.findById(pet.getOwner_id());
        if (owner != null) {
            if (owner.getPets() == null) {
                owner.setPets(new ArrayList<>());
            }
            ...
            for (OwnerPet p : owner.getPets()) {
                if (p.getId() != null && p.getId().equals(ownerPet.getId())) {
                    // Atualizar o pet existente
                    p.setName(ownerPet.getName());
                    p.setBirthDate(ownerPet.getBirthDate());
                    p.setType_name(ownerPet.getType_name());
                    petExists = true;
                    break;
                }
            }
            ...
            if (!petExists) {
                ...
                System.out.println("Pet adicionado à lista do owner: " + ownerPet.getName());
            }
            ...
            try {
                ...
                for (Pet.Visit petVisit : visits) {
                    ...
                    System.out.println("Visita salva para o pet ID: " + ownerPet.getId() + 
                                      ", Descrição: " + ownerVisit.getDescription());
                }
    ...
    public void savePet(OwnerPet pet) {
        ...
        if (pet.getId() != null) {
            Optional<OwnerPet> existingPetOpt = ownerPetRepository.findById(pet.getId());
            if (existingPetOpt.isPresent()) {
                OwnerPet existingPet = existingPetOpt.get();
                // Garantir que o owner_id não seja alterado durante a atualização
                if (!existingPet.getOwner_id().equals(pet.getOwner_id())) {
                    System.out.println("AVISO: Tentativa de alterar owner_id do pet de " + 
                                      existingPet.getOwner_id() + " para " + pet.getOwner_id() + 
        ...
        System.out.println("Pet salvo: " + pet.getName() + " (ID: " + pet.getId() + 
        ...
        if (owner != null) {
            ...
            for (OwnerPet p : owner.getPets()) {
                if (p.getId() != null && p.getId().equals(pet.getId())) {
                    ...
                    p.setType_name(pet.getType_name());
                    petExists = true;
                    break;
                }
            }
            ...
            if (!petExists) {
                owner.getPets().add(pet);
                System.out.println("Pet adicionado à lista do owner: " + pet.getName());
            }
